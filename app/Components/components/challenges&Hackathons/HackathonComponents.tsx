        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 max-w-12xl mr-10 ml-10">
            {currentActivities.map((activity) => (
              <div key={activity._id.$oid} className="relative border shadow-lg overflow-hidden max-w-sm bg-white rounded-3xl">
                <div className="h-48 w-full relative mb-2">
                  <Image src={umuravaBg} alt="Challenge Image" layout="fill" objectFit="cover" className="object-cover p-2 rounded-3xl" />
                  <div className={`absolute top-4 right-4 text-white text-sm font-semibold px-2 py-1 rounded z-10 ${activity.status === 'ongoing' ? 'bg-orange-500' : 'bg-green-500'}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2 mt-1">
                    {Array.isArray(activity.skills) &&
                      activity.skills.map((skill) => (
                        <span key={skill} className="border border-blue-500 text-blue-500 rounded-full px-3 py-1 text-sm">
                          {skill}
                        </span>
                      ))}
                  </div>
                  ...
                  <div className="mt-4 border-t border-gray-300 pt-4">
                    <Link href={`/view/${activity._id.$oid}`}>
                      <button className="bg-blue-600 text-white py-2 px-4 rounded">View Challenge</button>
                    </Link>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-center mt-6 space-x-4">
            <button
              ...
              className={`bg-gray-300 text-gray-700 py-2 px-4 rounded ${currentPage === 1 ? "cursor-not-allowed" : ""}`}
            >
              Previous
            </button>
            <button
              ...
              className={`bg-blue-600 text-white py-2 px-4 rounded ${currentPage === totalPages ? "cursor-not-allowed" : ""}`}
            >
              Next
            </button>
          </div>
